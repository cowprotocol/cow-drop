...
Four things ship from this repo: three container images and one npm package.
...
| `ghcr.io/cowprotocol/cow-drop/web` | The demo page, static behind nginx. | GitHub Container Registry |
...
- [`docker.yml`](../.github/workflows/docker.yml) — builds and pushes all three images, tagged
  `0.1.0`, `0.1`, `latest` and `sha-<commit>`.
...
Configured at **container start**, not at build time, so one image is promotable across
...
docker run -d --name cow-drop-web \
  ...
  -e RPC_URL=https://rpc.gnosischain.com \
  ghcr.io/cowprotocol/cow-drop/web:latest
```
