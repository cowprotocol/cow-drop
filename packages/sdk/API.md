...
tighten. All default `receiver` to the `owner`; pass the zero address to leave proceeds in the drop.

Each `steps.*` builder takes only the deployment field naming the contract that hosts it —
`Pick<DropDeployment, 'guardSteps'>` for a guard, and so on. A step's target is part of the drop address,
so asking a guard for `deployment.twapSteps` would misstate what moves that guard's address.
...
`OrderPlacement` is **CoW's own event**, not cow-drop's. Decoding is against `ONCHAIN_ORDERS_ABI` rather
than any particular emitter's ABI — EthFlow's own logs decode here — and an indexer filters on
`ORDER_PLACEMENT_TOPIC` alone. See
[`packages/watch-tower/ARCHITECTURE.md`](../watch-tower/ARCHITECTURE.md).

## Order hashing

The announcement carries no order uid: the owner is in `sender` (pre-signed) or `signature.data`
(ERC-1271), and the digest is the consumer's to compute.
...
This is a second implementation of a formula whose ground truth is Solidity, and the drift is silent — a
wrong digest means checking the pre-signature of an order that never existed. Every function here is
asserted against fixtures generated by the compiled contract.
...
> **The recipe file is the key, not the commitment.** The compiled `setupData` bytes are what the address
> commits to; the file is the only reproducible way back to them. Change any value — the label, the `once`
> flag, one digit of the price — and you get a different address.
>
> **Fund a drop, lose its recipe before activating it, and nobody can recover the money — the owner
> included**, since `initializeProxyWithoutSetup` needs the same bytes as everyone else and `DropTriggered`
> emits only their hash. (After a first activation the bytes are recoverable from the deploying
> transaction's calldata, which helps only in hindsight.)

> **Guards are steps, not settings.** `requireMinBalance` and `requireTimeWindow` are committed into the
> address, so nobody activating your drop can skip them. Put a `minAmount` on any one-shot recipe a bridge
> might fund in tranches; without it, the first tranche sizes the whole schedule. Ordering affects what a
> guard measures, not whether it binds — the recipe is atomic, so a guard anywhere unwinds the whole
> activation.

**`salt` and `orderSalt` are different things.** The recipe's `salt` is the factory's user salt: it moves
the drop address, so the same parameters can yield more than one drop (or grind a vanity address without
putting junk in `label`). The `twapFromBalance` step's `orderSalt` is the ComposableCoW conditional-order
discriminator and does not affect the address.

## Reading a recipe back

`describeRecipe(setupData, deployment)` turns committed bytes into named steps with decoded arguments.
Activation is permissionless and unsigned, so the *only* safeguard for someone about to send money to a
drop is re-deriving the address from a recipe they understand.

The output to read is `warnings`, per step. A step it cannot name is not an error — `raw` exists on purpose
— but it is reported as a gap rather than rendered as though understood:
...
`DropRecipeJson.generation` says which deployment of the contracts a file was compiled against. Every
address in a `DropDeployment` feeds the drop's CREATE2 preimage, so the same file against a later
generation yields a *different* address.

It defaults to **1**, not `LATEST_GENERATION`: a file written before the field existed was compiled against
generation 1, and defaulting to the latest would silently repoint it. Everything this package exports pins
the field explicitly. See [docs/DEPLOYMENTS.md](../../docs/DEPLOYMENTS.md).

## Generated constants, and tests

`src/generated/` comes from the foundry build via `scripts/generate-constants.mjs` — run `pnpm generate`
after any contract change. It reads every `contracts/deployments/gen*/` directory, so past generations
survive a redeploy.

`derivation.test.ts` checks this package's off-chain address derivation against fixtures generated by the
compiled contract (`contracts/script/Fixtures.s.sol`). Two implementations of one formula drift silently,
and the failure mode is an address that funds get stranded at.
