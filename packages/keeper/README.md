...
*notice* the funds arrived and send the transaction. Today that is a person with the page open,
clicking **Activate**. This does it unattended, for drops whose owner explicitly asked it to.

```bash
pnpm --filter @cowprotocol/cow-drop-keeper build

KEEPER_PRIVATE_KEY=0x… cow-drop-keeper --rpc-url $RPC_URL --state ./gnosis.json --port 8787
```
