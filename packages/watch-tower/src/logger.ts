...
/**
 * `HH:MM:SS level name  message`, one line per event, UTC.
 *
 ...
 */
export function createLogger({ name, quiet = false }: LoggerOptions): Logger {
  const line = (level: string, message: string) =>
    `${new Date().toISOString().slice(11, 19)} ${level.padEnd(5)} ${name}  ${message}`
